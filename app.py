...
def check(email):
    url = "https://ap4qpn0jb3.execute-api.us-east-2.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://ap4qpn0jb3.execute-api.us-east-2.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://ap4qpn0jb3.execute-api.us-east-2.amazonaws.com/plasma/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://ap4qpn0jb3.execute-api.us-east-2.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
    for i in a:
        url="https://www.fast2sms.com/dev/bulkV2?authorization=902ueKral5vcjfxmiIyz6sB4ZwOQNFXnJMHPSW1A8pg3ECVkGoPiGkZsdWxb3vMrR8tJpcBfEF0ToqeA&sender_id=TXTIND&message=Need plasma of your blood group&language=english&route=v3&numbers="+str(i['phone'])
